...
void coloriage_glouton(graphe const *g, uint8_t *couleur_sommet)
{
    ...
    for (int i = 0; i < g->ordre; i++)
    {
        ...
        for (int z = 0; z < 255; z++)
        {
            couleur_disponible[z] = 1;
        }

        if (couleur_sommet[i] == 255)
        {
            nb_sommet_adjacent = sommets_adjacents(g, i, sa);

            if (nb_sommet_adjacent != 0)
            {
                for (int j = 0; j < nb_sommet_adjacent; j++)
                {
                    couleur_disponible[couleur_sommet[sa[j]]] = 0;
                }
            }

            for (int k = 0; k < 255; k++)
            {
                if (couleur_disponible[k] == 1)
                {
                    couleur_sommet[i] = k;
                    break;
                }
...
void prim(graphe const *g, double const *poids_arete, graphe *acm) {
    for (size_t i = 0; i < ordre(g); i++) {
        ajouter_sommet(acm);
    }
    bool *visite = malloc(ordre(g) * sizeof(bool));
    ...
    while (true) {
        ...
        for (size_t i = 0; i < ordre(g); i++) {
            if (visite[i]) {
                ...
                for (size_t j = 0; j < nb_sommets; j++) {
                    if (!visite[sa[j]] && poids_arete[index_arete(g, (arete) {i, sa[j]})] < min) {
                        min = poids_arete[index_arete(g, (arete) {i, sa[j]})];
                        a_min = (arete) {i, sa[j]};
                    }
                }
            }
        ...
        visite[a_min.s2] = true;
        ajouter_arete(acm, a_min);
    }
    free(visite);
}
